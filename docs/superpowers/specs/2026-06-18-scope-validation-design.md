...
- **C1. Tenant isolation is opt-in and fails OPEN.** `backend/app/Shared/Tenancy/BelongsToTenant.php:17-22` applies the global scope only `if ($ctx->has())`. Any model that omits the trait has *zero* isolation, silently; and queue jobs / console commands / platform-admin requests (no resolved tenant) run tenant-scoped queries **unfiltered across all tenants**. GREEN today only by discipline. Fix: a base `TenantModel`, an architecture test asserting every tenant-owned model uses the trait, and fail-closed scoping outside an audited platform-admin/system allowlist. Directly threatens invariant #7.
...
- **C3. `$guarded = []` (full mass-assignment) on `Program` and `Cohort`** (`Program.php:17`, `Cohort.php:17`). Safe only while every write is a curated FormRequest. Tighten to `$fillable`/explicit casts.
